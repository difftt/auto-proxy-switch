...
新增可选模式：当检测到当前正在使用的美国节点质量不是 `good` 时，允许脚本自动切换到更优的可用美国节点。
...
8. 在自动切换模式下，只有当前节点检测等级不是 `good`，才开始检测其它美国候选节点并切换到更优可用节点。
9. 自动切换只在“当前节点不是 `good`”时触发，当前节点为 `good` 时不得因为存在更快节点而切换。
...
为了支持“当前节点质量不是 `good` 时自动切换”，脚本需要识别当前正在使用的美国节点。
...
只有当前节点等级不是 good 时才检测其它候选节点并允许切换；
