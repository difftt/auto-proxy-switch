...
用于检查 Clash Verge Rev / mihomo 中指定地区代理节点的实时状态、目标 API 可达性，并可在当前节点达到策略确认条件时尝试切换到更优节点。默认地区为美国。
...
兼容入口（保留旧版调用方式，等价于默认 `--region us`）：

```text
check_us_proxy_status.py
```

`check_us_proxy_status.py` 是不带 `--region` 时自动注入 `--region us` 的薄包装，原有 cron、脚本、文档中使用旧文件名的命令可继续工作。
...
```bash
./check_proxy_status.py --no-default-targets
```
...
> 上述命令中的 `./check_us_proxy_status.py` 也仍然可用：旧入口是会自动注入 `--region us` 的薄包装。
...
```cron
*/5 * * * * /usr/bin/python3 /path/to/auto-proxy-switch/check_proxy_status.py --auto-switch-if-current-not-good --switch-check-target discord --json >> /path/to/auto-proxy-switch/check_proxy_status.log 2>&1
```

每 1 分钟检查一次：

```cron
* * * * * /usr/bin/python3 /path/to/auto-proxy-switch/check_proxy_status.py --auto-switch-if-current-not-good --switch-check-target discord --json >> /path/to/auto-proxy-switch/check_proxy_status.log 2>&1
```

> 旧版 `check_us_proxy_status.py` 兼容入口在 cron 中同样可用，无需修改现有 crontab。
...
```bash
python3 -m py_compile check_proxy_status.py check_us_proxy_status.py
```
